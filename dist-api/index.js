...
const toDecimalString = (value, digits, fallback) => {
...
const toSlug = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
const buildProductSlug = (name, idOlist, explicitSlug) => {
    ...
    if (Array.isArray(list)) {
        for (const item of list) {
            ...
            if (item && typeof item === 'object') {
                const url = pickFirst(item, ['url', 'src', 'link']);
...
const normalizeOlistProduct = (raw) => {
...
const getOlistAuthorizationHeader = (token) => {
    const trimmed = token.trim();
    if (/^(JWT|Bearer)\s+/i.test(trimmed))
        return trimmed;
    return `JWT ${trimmed}`;
};
const getProvidedSyncSecret = (req) => {
    const querySecret = toSingleValue(req.query.secret);
    const headerSecret = toSingleValue(req.headers['x-sync-secret']);
    const authHeader = toSingleValue(req.headers.authorization);
    const bearer = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : authHeader;
    return querySecret || headerSecret || bearer;
};
const isSyncAuthorized = (req) => {
...
const fetchOlistProductsPage = async (page) => {
    ...
    const hasMoreFromPayload = Boolean(payload?.has_more
        ?? payload?.hasNextPage
        ?? payload?.next
        ?? payload?.next_page
        ?? (payload?.pagination && payload.pagination.has_next));
    const totalPages = toInt(payload?.total_pages
        ?? (payload?.pagination && payload.pagination.total_pages), 0);
    const hasMore = totalPages > 0 ? page < totalPages : (hasMoreFromPayload || items.length >= olistConfig.pageSize);
    return { items, hasMore };
};
const syncOlistProducts = async (maxPagesInput) => {
    if (!hasOlistConfig) {
...
const runOlistSync = async (maxPages) => {
    if (syncInFlight)
        return syncInFlight;
...
// --- OLIST SYNC ---
apiRouter.get('/integrations/olist/status', async (_req, res) => {
    res.json({
...
apiRouter.post('/integrations/olist/sync', async (req, res) => {
...
// --- CATEGORIAS ---
apiRouter.get('/categorias', async (_req, res) => {
    try {
